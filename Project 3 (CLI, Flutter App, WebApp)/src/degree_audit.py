"""
degree_audit.py  —  Level 3 · Degree Audit & Deficiency Reporter
══════════════════════════════════════════════════════════════════
...
Usage:  python degree_audit.py <transcript.csv> <CSE|LLB> [knowledge_base.md]
"""
...
from core import (
    ...
    table_header, table_row, progress_bar, big_number,
...
def main():
    if len(sys.argv) < 3:
        print(f"\n  {C.ROSE}{C.BOLD}Usage:{C.RST}  python degree_audit.py "
              f"{C.GOLD}<transcript.csv> <CSE|LLB>{C.RST} "
              f"{C.GRAY}[knowledge_base.md]{C.RST}")
        sys.exit(1)
    ...
    if program not in ("CSE", "LLB"):
        print(f"\n  {C.ROSE}{C.BOLD}✗ Error:{C.RST} Unknown program '{program}'. Use CSE or LLB.")
        sys.exit(1)
    ...
    for i, code in enumerate(mandatory, 1):
        if code in completed:
            table_row(
                ...
                widths,
                [C.GRAY, C.WHITE, C.MINT],
            )
        else:
            table_row(
                [f"{i:>3}.", code, "✗ Missing"],
                widths,
                [C.GRAY, C.ROSE, C.ROSE],
            )
    ...
    if choice_groups:
        ...
        for cg in choice_groups:
            ...
            if taken:
                cg_satisfied += 1
                print(f"  {C.MINT}{C.BOLD}  ✓{C.RST}  {C.WHITE}{label:<38}{C.RST} "
                      f"{C.GRAY}({cr:.0f} cr){C.RST}  →  {C.MINT}{taken[0]}{C.RST}")
            else:
                print(f"  {C.ROSE}{C.BOLD}  ✗{C.RST}  {C.WHITE}{label:<38}{C.RST} "
                      f"{C.GRAY}({cr:.0f} cr){C.RST}  →  {C.ROSE}None taken{C.RST}")
    ...
    if program == "CSE":
        ...
        for trail_name, matched in trail_hits.items():
            if matched:
                ...
                print(f"  {C.MINT}●{C.RST}  {C.TEAL}{trail_name:<44}{C.RST}"
                      f" {C.MINT}{cnt} course(s), {cr:.0f} cr{C.RST}")
                for c in matched:
                    print(f"      {C.WHITE}{c['code']:<10}{C.RST} {C.GRAY}{c['name']}{C.RST}")
            else:
                print(f"  {C.GRAY}○  {trail_name:<44}{C.RST} {C.GRAY}0 courses{C.RST}")
        ...
        if trail_rule_ok and credit_rule_ok:
            print(f"\n  {C.MINT}{C.BOLD}  ✓ Trail rule satisfied{C.RST}"
                  f" {C.GRAY}(≥2 from one trail + ≥1 from another){C.RST}")
            print(f"  {C.MINT}{C.BOLD}  ✓ Credit rule satisfied{C.RST}"
                  f" {C.GRAY}({total_elective_cr:.0f}/9 credits){C.RST}")
        else:
            elective_ok = False
            if not trail_rule_ok:
                print(f"\n  {C.ROSE}{C.BOLD}  ✗ Trail rule NOT met{C.RST}"
                      f" {C.GRAY}(need ≥2 from one trail + ≥1 from another){C.RST}")
        ...
        if open_cr >= 3:
            print(f"\n  {C.MINT}{C.BOLD}  ✓ Open elective requirement met{C.RST}"
                  f" {C.GRAY}({open_cr:.0f} credits){C.RST}")
        else:
        ...
        # ── LLB Law Electives (Pick 4 from pool) ─────────────────────────
        if prog["elective_pools"]:
            pool = prog["elective_pools"][0]
            pick = pool["pick"]

            section(f"Law Elective Audit  (Pick {pick}, 12 Credits)", "⚖️")

            matched = []
            for pc in pool["courses"]:
                if pc["code"] in completed:
                    matched.append(pc)
                    print(f"  {C.MINT}●{C.RST}  {C.WHITE}{pc['code']:<10}{C.RST}"
                          f" {C.MINT}{pc['name']}{C.RST}")
                else:
                    print(f"  {C.GRAY}○  {pc['code']:<10}{C.RST}"
                          f" {C.GRAY}{pc['name']}{C.RST}")

            separator(style="dots")
            elec_cr = sum(c["credits"] for c in matched)
            if len(matched) >= pick:
                print(f"\n  {C.MINT}{C.BOLD}  ✓ Law elective requirement met{C.RST}"
                      f" {C.GRAY}({len(matched)}/{pick} courses, {elec_cr:.0f} cr){C.RST}")
            else:
                elective_ok = False
                shortfall = pick - len(matched)
                print(f"\n  {C.ROSE}{C.BOLD}  ✗ Need {shortfall} more law elective(s){C.RST}"
                      f" {C.GRAY}({len(matched)}/{pick}){C.RST}")
            spacer()

        # ── Free Electives (6 cr) ─────────────────────────────────────────
        section("Free Electives  (6 Credits)", "📂")
        known_codes = set(prog["mandatory"])
        for cg in choice_groups:
            known_codes.update(cg["options"])
        for sec in prog["sections"]:
        ...
        if free_cr >= 6:
            print(f"\n  {C.MINT}{C.BOLD}  ✓ Free elective requirement met{C.RST}"
                  f" {C.GRAY}({free_cr:.0f} credits){C.RST}")
        else:
            print(f"\n  {C.GOLD}{C.BOLD}  ○ Free electives:{C.RST} {free_cr:.0f}/6 credits")
            if free_cr < 6:
                elective_ok = False
        spacer()
    ...
    if retakes:
        ...
        for code, grades in retakes.items():
            arrow = f" {C.GRAY}→{C.RST} ".join(
                f"{grade_color(g)}{g}{C.RST}" for g in grades
            )
            final = grades[-1]
            print(f"  {C.WHITE}  {code:<10}{C.RST} {arrow}  "
                  f"{C.GRAY}(final: {grade_color(final)}{C.BOLD}{final}{C.RST}"
                  f"{C.GRAY}){C.RST}")
        spacer()
    ...
    stat_card("Remaining", f"{remaining:.1f}", C.SKY, "○")
    ...
    standing, _ = get_academic_standing(cgpa)
    ...
    if waiver_pct > 0:
        ...
        stat_card("Waiver", waiver_label, C.GRAY, "○")
    spacer()
