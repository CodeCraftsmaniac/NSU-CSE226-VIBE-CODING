...
| **LLB** | Law | Law degree — 130 credits, 23 mandatory courses, pick-4 law electives |
...
│   └── llb/                          LLB department data
│       ├── transcript.csv            The "real" LLB student transcript (29 rows)
│       ├── test_L1.csv               Test file for Level 1 edge cases
│       ├── test_L2.csv               Test file for Level 2 CGPA math
│       └── test_retake.csv           Test file for retake scenarios
│
...
│   └── llb/                          6 batch files for LLB
│       ├── run_level1.bat            Runs Level 1 with LLB transcript
│       ├── run_level2.bat            Runs Level 2 with LLB transcript
│       ├── run_level3.bat            Runs Level 3 with LLB transcript
│       ├── run_test_L1.bat           Runs Level 1 with test_L1.csv
│       ├── run_test_L2.bat           Runs Level 2 with test_L2.csv
│       └── run_test_retake.bat       Runs Level 3 with test_retake.csv
│
...
- **`data/cse/` and `data/llb/`** — Each department has its own folder so transcripts don't mix.
- **`scripts/cse/` and `scripts/llb/`** — Faculty can just double-click a `.bat` file. No need to type commands.
...
1. Open the `scripts/cse/` or `scripts/llb/` folder in File Explorer
...
# ── LLB Department ──
python src/credit_engine.py data/llb/transcript.csv              # Level 1
python src/cgpa_analyzer.py data/llb/transcript.csv              # Level 2
python src/degree_audit.py data/llb/transcript.csv LLB knowledge_base.md  # Level 3
...
| `degree_audit.py` | 400 | `<transcript.csv> <CSE\|LLB> [knowledge_base.md]` | Full degree audit + graduation verdict |
...
2. For each program:
   ...
   - Sections matching "Pick N" → become **elective pools** (LLB only)
...
1. **Parse arguments** — needs 2-3 arguments: CSV path, program code (CSE or LLB), optionally knowledge base path (defaults to `knowledge_base.md`).
...
5. **Print banner** — "DEGREE AUDIT · CSE" or "DEGREE AUDIT · LLB"
...
9. **AUDIT 3 — Electives (different per program):**
   ...
   **For LLB:**
   - **Law Electives (12 credits):** There's a pool of 9 law elective courses. Student must pick any 4 (3 credits each = 12 credits).
...
```
👤  Student Overview
📋  Mandatory Course Audit (44 courses for CSE / 23 for LLB)
🔀  Choice-Group Audit
🎯  CSE Major Elective Audit / ⚖️ Law Elective Audit
📂  Open/Free Electives
🔄  Retake History
📊  Credit Progress
...
`knowledge_base.md` is a Markdown file (286 lines) that contains the complete curriculum for both CSE and LLB programs. It's the "brain" that Level 3 audits against. The Python code NEVER hardcodes course lists — it reads them from this file.
...
(same structure for LLB)
```

### How the parser reads it

1. **Program detection:** Splits on `## [Program: ...]` headers. If name contains "CSE" → key is `CSE`. If "LLB" or "LAW" → key is `LLB`.
...
**`data/llb/transcript.csv`** — An LLB student who:
...
| `scripts/llb/` | `run_level1.bat` | Level 1 (credit tally) | `data/llb/transcript.csv` |
| | `run_level2.bat` | Level 2 (CGPA + waiver) | `data/llb/transcript.csv` |
| | `run_level3.bat` | Level 3 (degree audit) | `data/llb/transcript.csv` |
| | `run_test_L1.bat` | Level 1 test | `data/llb/test_L1.csv` |
| | `run_test_L2.bat` | Level 2 test | `data/llb/test_L2.csv` |
| | `run_test_retake.bat` | Level 3 test | `data/llb/test_retake.csv` |
...
| Unknown program code | Red error + exit if not "CSE" or "LLB" | `degree_audit.py` → program check |
...
2. **Update `degree_audit.py`:**
   - Change the program check from `if program not in ("CSE", "LLB")` to include `"BBA"`
...
**LLB Department second:**

4. **Double-click `scripts/llb/run_level1.bat`**
   - Point out: different department (Law), no retakes
   - Point out: 97 credits earned (more than CSE student)

5. **Double-click `scripts/llb/run_level2.bat`**
   - Point out: CGPA 3.29 — Good Standing
   - Interactive waiver prompt

6. **Double-click `scripts/llb/run_level3.bat`**
   - Point out: different audit structure (law electives instead of trails)
...
A: The spec requires programs from different departments. CSE is in ECE (Engineering), LLB is in Law.
