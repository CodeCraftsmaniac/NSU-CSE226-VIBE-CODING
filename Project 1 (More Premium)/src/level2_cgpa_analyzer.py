"""
level2_cgpa_analyzer.py  —  Level 2 · CGPA & Waiver Handler
════════════════════════════════════════════════════════════
...
Usage:  python level2_cgpa_analyzer.py <transcript.csv>
"""
...
from core import (
    ...
    classify_credit, grade_color, grade_pill, _grade_rank,
    banner, section, info_line, stat_card, separator, spacer,
    table_header, table_row, big_number, progress_bar, panel,
...
def main():
    if len(sys.argv) < 2:
        print(f"\n  {C.ROSE}{C.BOLD}Usage:{C.RST}  python level2_cgpa_analyzer.py {C.GOLD}<transcript.csv>{C.RST}")
        sys.exit(1)
    ...
    for i, r in enumerate(resolved, 1):
        ...
        if not grade:
            gp_str, qp_str, qp_clr = "—", "—", C.SLATE
        elif grade in NON_GPA_GRADES:
            gp_str, qp_str, qp_clr = "—", "—", C.SLATE
        else:
        ...
        table_row(
            [f"{i:>3}.", code, display_grade, f"{credits:.1f}", gp_str, qp_str],
            widths,
            [C.SLATE, C.FROST, gclr, C.FROST, gclr, qp_clr],
        )
    ...
    stat_card("Total Quality Points", f"{total_qp:.2f}", C.ELECTRIC, "◆")
    stat_card("GPA-Eligible Credits", f"{total_gpa_cr:.1f}", C.ELECTRIC, "◆")
    ...
    if cgpa >= 3.90:
        ...
        standing, st_clr, st_icon = "Cum Laude", C.ELECTRIC, "★★"
    elif cgpa >= 2.00:
    ...
    print(f"\n  {C.ELECTRIC}{C.BOLD}  The admin may grant waivers for specific courses.{C.RST}")
    print(f"  {C.SLATE}  Waived courses are exempted from requirements but do{C.RST}")
    print(f"  {C.SLATE}  NOT earn credits or affect GPA.{C.RST}")
    print()
    print(f"  {C.GOLD}{C.BOLD}  ? Waivers granted for any courses?{C.RST}")
    print(f"  {C.SLATE}    Enter course codes separated by commas (e.g. ENG102, BUS112){C.RST}")
    print(f"  {C.SLATE}    Or press Enter to skip:{C.RST}")

    try:
        waiver_input = input(f"  {C.AQUA}{C.BOLD}  ▸ {C.RST}").strip()
    ...
    if waived_courses:
        separator(style="dots")
        stat_card("Waived Courses", ", ".join(waived_courses), C.AQUA, "◆")
        stat_card("Waiver Count", f"{len(waived_courses)} course(s)", C.AQUA, "◆")
        stat_card("Adjusted Status", "Requirements now satisfied", C.MINT, "✓")
        print(f"\n  {C.SLATE}  Note: Waived courses satisfy requirements but do not{C.RST}")
        print(f"  {C.SLATE}  contribute to earned credits or CGPA.{C.RST}")
    else:
        print(f"\n  {C.SLATE}  No waivers applied.{C.RST}")
    spacer()
    ...
    if retakes:
        ...
        cols2   = ["Course", "Journey", "Best", "Impact"]
        widths2 = [10, 18, 8, 18]
        table_header(cols2, widths2)

        for code, grades in retakes.items():
            journey = " → ".join(g if g else "IP" for g in grades)
            best = max(grades, key=lambda g: _grade_rank(g))
            first = grades[0]
            best_gp = GRADE_POINTS.get(best)
            first_gp = GRADE_POINTS.get(first)

            # Handle non-GPA starting grades (W, I, P, TR, empty)
            if first_gp is None or best_gp is None:
                if best_gp is not None:
                    change_str = f" {best_gp:.2f}  new grade"
                    ch_clr = C.ELECTRIC
                else:
                    change_str = "  —   non-GPA"
                    ch_clr = C.SLATE
            else:
                diff = best_gp - first_gp
                if diff > 0:
            ...
            table_row(
                [code, journey, best if best else "IP", change_str],
                widths2,
                [C.FROST, C.SLATE, grade_color(best), ch_clr],
            )
    ...
    if waiver_pct > 0:
        ...
        print(f"     {C.AQUA}{waiver_label}{C.RST}")
    else:
        print(f"\n  {C.GOLD}{C.BOLD}  ○ {waiver_label}{C.RST}")
        if total_earned < 30:
            print(f"     {C.SLATE}Complete at least 30 credits to be considered.{C.RST}")
        else:
            print(f"     {C.SLATE}Raise CGPA to 3.50+ for Dean's Scholarship.{C.RST}")

    # Scholarship ladder
    print()
    thresholds = [
        (3.50, "25%", "Dean's Scholarship", C.ELECTRIC),
    ...
    for threshold, pct_str, label, clr in thresholds:
        if cgpa >= threshold:
            ...
            marker = f"{C.SLATE}○{C.RST}"
        print(f"  {marker}  {C.FROST}≥ {threshold:.2f}{C.RST}  →  {clr}{C.BOLD}{pct_str:<5}{C.RST} {C.SLATE}{label}{C.RST}")
